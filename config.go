...
const remoteConfigURL = "https://gitee.com/a233s/spark_url/raw/master/url.txt"

// fetchConfig fetches configuration from the specified URL
func fetchConfig() error {
	...
	// Define a structure matching the response
	var result struct {
		WebURL struct {
			Secure bool   `json:"Secure"`
			Host   string `json:"Host"`
	...
	// Populate Config with the fetched data
	Config.Secure = result.WebURL.Secure
	Config.Host = result.WebURL.Host
	Config.Port = result.WebURL.Port
	Config.Path = result.WebURL.Path

	return nil
}

func GetBaseURL(ws bool) string {
	// Ensure configuration is fetched before generating the URL
